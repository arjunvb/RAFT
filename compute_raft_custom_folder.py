...
def infer_optical_flows(model, image_dir, output_dir, skip_exists=False):
    ...
    flow_bk_save_path = os.path.join(output_dir, "flow_b")
    if not os.path.exists(flow_bk_save_path):
        os.makedirs(flow_bk_save_path)
    flow_img_save_path = os.path.join(output_dir, "flow_imgs")
    ...
    with torch.no_grad():
        ...
        for img_id in tqdm(range(n_images - 1)):
            # output name
            imfile1, imfile2 = images[img_id], images[img_id + 1]
            flow_save_name = os.path.join(flow_save_path, imfile1.split('/')[-1].replace('.png', '.flo').replace('.jpg', '.flo'))
            flow_bk_save_name = os.path.join(flow_bk_save_path, imfile1.split('/')[-1].replace('.png', '.flo').replace('.jpg', '.flo')).replace('.flo', '-bk.flo')
            img_save_name = os.path.join(flow_img_save_path, imfile1.split('/')[-1])
            if skip_exists:
                if os.path.exists(flow_save_name) and os.path.exists(flow_bk_save_name) and os.path.exists(img_save_name):
                    continue
            ...
            flow_forward, flow_backward = padder.unpad(flow_up), padder.unpad(flow_up_bk)
...
if __name__ == '__main__':
    ...
    compute_raft_custom_folder(args.image_dir, args.output_dir, args=args, skip_exists=args.skip_exists)
